...
void NodeFaceIdentification::processFaces()
{
  ...
  for (const auto & [face_id, face] : tracked_faces_) {
    if (auto face_aligned = face->aligned()) {
      ...
      if (identify_all_faces_ || !face_persons_map_.count(face_id)) {
        ...
        if (!results.empty()) {
          ...
          if (new_person_created) {
            ...
            last_logged_match_[face_id] = results.front().first;
          } else {
            if (!last_logged_match_.count(face_id) || last_logged_match_[face_id] != results.front().first) {
              RCLCPP_INFO(
                this->get_logger(),
                fmt::format(
      ...
      face_persons_map_.erase(face_id);
      last_logged_match_.erase(face_id);
    }
  }
