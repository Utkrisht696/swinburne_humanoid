...
class AlsaMicPublisher(Node):
    def __init__(self):
        ...
        self.declare_parameter('vad_rms_threshold', 120.0)
        self.declare_parameter('vad_hold_frames', 6)
        ...
        self.vad_rms_threshold = float(self.get_parameter('vad_rms_threshold').value)
        self.vad_hold_frames = int(self.get_parameter('vad_hold_frames').value)
        ...
        if self.input_rate != 48000 or self.output_rate != 16000:
            self.get_logger().warning(
                f'This script is intended for 48000 -> 16000. Current: '
                f'{self.input_rate} -> {self.output_rate}'
            )
    ...
    def timer_callback(self):
        ...
        try:
            ...
            if not self._shutting_down:
                self.get_logger().error(f'ALSA read failed: {exc}')
            return
        ...
        while self.output_buffer.size >= self.output_chunk:
            ...
            if rms >= self.vad_rms_threshold:
                self._vad_frames_remaining = self.vad_hold_frames
            elif self._vad_frames_remaining > 0:
                self._vad_frames_remaining -= 1

            vad_state = self._vad_frames_remaining > 0
            ...
            if self.log_every_n > 0 and self._pub_count % self.log_every_n == 0:
                ...
                self.get_logger().info(
                    f'published={self._pub_count} RMS={rms:.1f} PEAK={peak} VAD={vad_state}'
                )
    ...
    def close_capture(self):
        ...
        if self._last_vad_state is not False:
            try:
